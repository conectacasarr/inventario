...
def relatorios():
    ...
    if filtro_busca:
        ...
        params_itens.extend([busca, busca])
    ...
    if filtro_data_inicio:
        where_clauses_itens.append("i.data_aquisicao >= ?")
    ...
    if filtro_tipo == "emprestados":
        where_clauses_itens.append("e.data_devolucao IS NULL")
    elif filtro_tipo == "devolvidos":
        where_clauses_itens.append("e.data_devolucao IS NOT NULL")
    ...
    query_emprestimos = """
        SELECT e.*, i.descricao, i.tombamento, i.quantidade, u.nome
        FROM emprestimos e
        JOIN emprestimo_itens ei ON e.id = ei.emprestimo_id
        JOIN itens i ON i.id = ei.item_id
    ...
    grupos_disponiveis = [row['nome'] for row in db.execute("""
        SELECT DISTINCT g.nome
        FROM itens i
        JOIN grupos g ON i.grupo_id = g.id
        WHERE g.nome IS NOT NULL AND g.nome != ''
    """).fetchall()]

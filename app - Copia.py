...
def relatorios():
    ...
    # Obter parâmetros de filtro
    filtro_grupo = request.args.get("grupo", "")
    filtro_tipo = request.args.get("tipo", "todos") # 'todos', 'emprestimos'
    ...
    # Construir cláusulas WHERE comuns
    where_clauses_itens = []
    params_itens = []
    if filtro_grupo:
        if filtro_grupo:
            where_clauses_itens.append("i.grupo_id = ?")
        params_itens.append(filtro_grupo)

    where_clauses_emprestimos = list(where_clauses_itens) # Copiar filtros de item
    params_emprestimos = list(params_itens)
    if filtro_data_inicio:
        where_clauses_emprestimos.append("e.data_emprestimo >= ?")
        params_emprestimos.append(f"{filtro_data_inicio} 00:00:00")
    if filtro_data_fim:
        where_clauses_emprestimos.append("e.data_emprestimo <= ?")
        params_emprestimos.append(f"{filtro_data_fim} 23:59:59")

    # Construir strings WHERE
    where_str_itens = " WHERE " + " AND ".join(where_clauses_itens) if where_clauses_itens else ""
    where_str_emprestimos = " WHERE " + " AND ".join(where_clauses_emprestimos) if where_clauses_emprestimos else ""

    # Buscar dados com base nos filtros
    # Sempre busca itens para referência, mesmo que não sejam exibidos diretamente
    query_itens_base = f"""
        SELECT i.tombamento, i.descricao, g.nome AS grupo, m.nome AS marca, i.valor_unitario, i.quantidade
        FROM itens i
        LEFT JOIN grupos g ON i.grupo_id = g.id
        LEFT JOIN marcas m ON i.marca_id = m.id
        {where_str_itens}
        ORDER BY i.tombamento
    """
    itens = db.execute(query_itens_base, params_itens).fetchall()
    #print([dict(item) for item in itens])
    ...
    if filtro_tipo in ["todos", "emprestimos"]:
        ...
        for emp in emprestimos_raw:
            e = dict(emp)  # transforma para dicionário
            e["tombamentos"] = e["tombamentos"].split(", ") if e["tombamentos"] else []
            e["descricoes"] = e["descricoes"].split(", ") if e["descricoes"] else []
            e["marcas"] = e["marcas"].split(", ") if e["marcas"] else []
            e["grupos"] = e["grupos"].split(", ") if e["grupos"] else []
            e["quantidades"] = e["quantidades"].split(", ") if e["quantidades"] else []
    ...
    if formato == "pdf":
        return exportar_pdf(itens, emprestimos, filtro_tipo, filtro_grupo, filtro_data_inicio, filtro_data_fim)
    
    # Calcular total geral dos itens para exibição no HTML
    total_geral_valor = sum((item["valor_unitario"] or 0) * item["quantidade"] for item in itens)
    total_geral_html = formata_brl(total_geral_valor)

            
    return render_template("relatorios_simples.html", 
                          itens=itens, # Enviar todos os itens filtrados por grupo para referência, se necessário 
